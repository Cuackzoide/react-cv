...
export const WebSummary = ({ professionalProfile, lang }) => {
  const role = {
    id: "web_developer",
    ...professionalProfile.roles.web_developer,
  };
              ...
              <motion.div>
                {role.core_focus.map((focus, index) => (
                  <motion.p key={index} className=" m-2">
                    <span className="text-primary fs-6 fw-semibold bg-warning-subtle rounded p-1">
                      {focus}
                    </span>
                  </motion.p>
                ))}
